...
(function(window, undefined) {
	...
	function monthpay(money, fee, times) {
		...
		fee = +fee;
		times = +times;
		return (money * fee * Math.pow(fee + 1, +(times)))
				/ (Math.pow(fee + 1, +(times)) - 1);
	}
	...
	function calc(inputpl, inputpla) {
		var l = {
			...
			f : 0.0035525,
			p : "201607"
		}, pl = {}, pla = [], keys = [ "期数", "还款日期", "当月还款", "计息剩余贷款", "当月利息",
				"当月还本", "结余" ], i = 0, result = [], rtext = "", syqs = l.q, zqs = l.q, mp = monthpay(
				l.m, l.f, zqs), mdic, year = "2016", month = "07", date, gz = 6666, //
		sydk = l.m, jgqs = 0, dylx, dybj, tq, tqsssydk, tqsssyqs, tqssdylx, tqssdybj, tqssjgqs, pc, tqdate, dqdate, nresult, recalc = false, firstre = true, lastdic, summp, sumlx, txt, rtx;

		if (inputpl != null && inputpl.length > 0 && inputpla != null
				&& inputpla.length > 0) {
			pl = inputpl;
			pla = inputpla;
		}
		...
		rtext = rtext + "总期数:," + result.length + ",总还:," + summp.toFixed(3)
				+ ",总利息:," + sumlx.toFixed(3) + "\r\n\r\n";
		...
		$("#resultregion").width(document.body.scrollWidth - 8).height(
				document.body.scrollHeight - 8).text(rtext);
	}
	window.$loan = {};
